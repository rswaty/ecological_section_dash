...
The first step to identifying RSAs (as outlined in Annex G) is to identify native ecosystems that would typically occur in the management unit given current climate and soil conditions. This ﬁrst section helps to identify the native ecosystems that should occur in the landscape given the existing climate and soil conditions (an essential element of Indicator 6.5.1), as well as existing ecosystems that are currently present within the landscape using [LANDFIRE](https://landfire.gov/) data.
...
### Native Ecosystems Greater than 1,000 acres
...
bps_name_chart <- 
  ...
  labs(
    subtitle = "",
    caption = "Biopphysical Settings (refered to as Native Ecosystems here) data obtained from https://landfire.gov/vegetation/bps.",
    x = "",
    y = "Acres") +
...
cat(
  paste0(
    "**Native Ecosystems that had less than 1,000 acres in this Ecological Section were:  ",
...
* LANDFIRE data shows there were **`r number_of_bps`** Native Ecosystems (by name, there may be more if counting by regional variants).  
...
For the charts below we assessed the amounts of each Existing Ecosystem (at a coarse level; includes agricultural and developed types) within each of the top 3 most prevalent Native Ecosystem footprints for this Ecological Section. More specifically, for each 30M x 30M cell (or pixel) we first mapped the Native Ecosystem, then we mapped what that same pixle was in 2024.  All of the possible combinations were then tallied up to make the charts below. 

<br>

**Understanding the Sankey charts:**

* The Native Ecosystem is on the left, coarse-level Existing Ecosystems on the right.
* Hover over the grey horizontal bands to explore how Native Ecosystems have split out to the associated Existing Ecosystems on the current landscape.  
...
In the table below we summarize conversion of the top 10 Native Ecosystems. All area values are in acres.  Click on column names to sort.
...
* We selected the top 10 Native Ecosystems by their unique numerical code for the charts below.  Most Native Ecosystems have 'variants' representing different reference conditions (e.g., different successional stages and amounts).  Therefore there may be multiple charts per BpS name, and each Native Ecosystem in the 'Native Ecosystems' chart above may not have a chart below.
...
* Because the succession classes are custom for each ecosystem the classes are labeled simply as A-E, plus:
...
To better understand 'reference' conditions for the Native Ecosystems of the US, the LANDFIRE program developed descriptions plus accompanying state-and-transition models that depict how these systems 'looked and functioned' with natural disturbance regimes.  Below you can download the descriptions for the dominant Native Ecosystems and explore the succession class descriptions in a table.
...
As noted above each Native Ecosystem has up to 5 succession classes, and each one has unique descriptions and mapping rules.  Those descriptions and mapping rules for the Native Ecosystems presented in the Succession Class Chart are presented below.
