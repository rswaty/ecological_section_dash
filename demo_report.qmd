...
number_of_bps <- length(unique(bps_scls_gap$bps_model))
...
top_bps_summary <- bps_evt_norm %>%
  ...
  summarise(
    total_area = round(sum(count, na.rm = TRUE) * 0.2223945),
    natural_area = round(sum(count[natural_veg], na.rm = TRUE) * 0.2223945),
...
bps_urls <- filtered_sclass %>%
  mutate(bps_model = as.character(bps_model)) %>%
  semi_join(section_bps_models, by = "bps_model") %>%
...
Importantly, these assessments focus on ecosystems greater than 1,000 acres. For lesser present ecosystems, we suggest reviewing additional data sources such as State Wildlife Action Plans, [NatureServe](https://www.natureserve.org/), or local conservation initiatives to ensure you have a full understanding of the landscape(s) in which your management unit occurs.

This section also provides Sankey charts for the top 3 most prevalent Historic Native Ecosystems, plus a conversion summary table for all Historic Native Ecosystem models in this Ecological Section.
...
* LANDFIRE data shows there were **`r number_of_bps`** Historic Native Ecosystem models (counting regional model variants separately).  
* The 10 largest Historic Native Ecosystems covered **`r bps_top10_pct`%** of total mapped area in this Ecological Section. 
* As of 2024 LANDFIRE mapped **`r number_evt`** Existing Ecosystems for this Ecological Section.
* The 10 largest Existing Vegetation Types (including agricultural, developed, and water types) covered **`r evt_top10_pct`%** of total mapped area in this Ecological Section. 
...
* The percent in parentheses on the left is that Historic Native Ecosystem's share of the three footprints shown (not of the full Ecological Section).
...
In the table below we summarize conversion of all Historic Native Ecosystem models in this Ecological Section. All area values are in acres. "Natural vegetation remaining" means not Agricultural or Developed (exotic vegetation is counted as remaining). Click on column names to sort.
...
top_bps_summary %>%
  ...
  datatable(
    ...
    options = list(
      ...
      scrollCollapse = TRUE,
      columnDefs = list(
        list(visible = FALSE, targets = 6)  # hide the numeric sort col
      )
    )
  ) %>%
...
formatStyle(
  columns = 1:6,   # only visible columns
  `text-align` = "center"
)
