...
evt_pad_summary <- evt_pad_summary |>
  mutate(
    # acres for EACH bar segment (evt × gap_sts)
    ...
    evt_label = factor(
      evt_label,
      levels = unique(evt_label[match(top_evt_names_df$evt_name, evt_name)])
    )
  )
...
plot <-
  ...
  geom_bar(stat = "identity", position = "stack", width = 0.6) +  # thinner bars
  coord_flip() +
  ...
  labs(
    fill = "Gap Status",
    subtitle = "Gap Status 1 -3 for top Existing Ecosystems",
...
plotly <-
  ggplotly(plot, height = 800, width = 1000, tooltip = "text") %>%
  layout(
...
htmltools::div(
  style = "height: 850px; margin-bottom: 20px;",
  plotly
)
