...
evt_pad_summary <- evt_gap_status_top10 |>
  ...
  dplyr::mutate(
    ...
    evt_name = forcats::fct_reorder(evt_name, pct_sum),
...
This first section helps to identify the native ecosystems that should occur in the landscape given the existing climate and soil conditions (an essential element of Indicator 6.5.1), as well as ecosystems that are currently present within the landscape. Importantly, these assessments only cover the top 10 most prevalent ecosystems.  Additionally, this section also provides tables of the top 3 ecosystem types that should occur given historic data broken down to identify percentage remaining or converted to currently present ecosystems.
...
evt_name_chart <- 
  ...
  geom_bar(stat = "identity", fill = "#4f4f4f") +
  labs(
    subtitle = "",
...
* There were **`r number_of_bps`** Biophysical Settings (by name, there may be more if counting by variants).  The **`r top_bps_name`** was the most prevalent overall.  The top 10 Native Ecosystems presented here covered **`r bps_top10_pct`%** of the total area. 
* Currently LANDFIRE maps **`r number_evt`** Existing Vegetation Types with the **`r top_evt`** being most dominant for this Ecological Section.  The top 10 Existing Ecosystems cover **`r evt_top10_pct`%** of the total area. 
...
This section provides an assessment of protection and representation of the above identified ecosystem types within (Gap Analysis Project)[https://www.usgs.gov/programs/gap-analysis-project] status lands [ACE4.1]within the landscape (another element of Indicator 6.5.1). It does not evaluate “adequate” vs “inadequate” levels of protection and representation, which will be unique to each identified ecosystem type and its distinct qualities. Additionally, it highlights native ecosystems that should occur in the landscape given historic context[ACE5.1] and current conditions but are not well represented and protected in GAP status 1, 2 and 3 lands [ACE6.1]within the landscape. 
...
## Exploration of Successional Classes and Historical Disturbance Amounts for top Native Ecosystesm

ADD FSC LANGUAGE.  RELEVANT TO RSAs, 6.1.1, etc. 
